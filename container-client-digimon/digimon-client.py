...
def run():
    # while True: Uncomment to have dataset start again
        with grpc.insecure_channel('digimon-server:50052') as channel:
            ...
            for pokemon in stub.RequestPokemon(PokemonServer_pb2.PokeRequest(numberOfPokemon=int(1))):
                ...
                print("Requesting Digimon", flush=True)
                ...
                try:
                    ...
                    # conn.flushall()
                    conn.set("D_mostCommonType",  mostCommonType)
                    conn.set("D_mostCommonTypeLen",  mostCommonTypeLen)
                    conn.set("D_averageHP",  averageHP)
                    conn.set("D_weakestAttack",  weakestAttack)
                    conn.set("D_strongestAttack",  strongestAttack)
                    conn.set("D_toughestPokemon",  toughestPokemon)
                    conn.set("D_toughestPokemonName",  toughestPokemonName)
                    conn.set("D_allPokemon", str(allPokemon))
                    conn.set("D_allHp", str(pokemon_hp))
                    conn.set("D_allAttack", str(pokemon_attack))
                    conn.set("D_allNames", str(pokemon_names))

                    if (time.time() - start_time) >= 180:    # If it's been 3 mins (180 Seconds) , remove one for every added type.
                        conn.set("D_rollingStatus", "Rolling has started")
                    else:
                        conn.set("D_rollingStatus", "Not 3 minues yet")

                    print("Sending Digimon Analytics", flush=True)
